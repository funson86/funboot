    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h2 class="card-title"><?= !is_null($this->title) ? Html::encode($this->title) : Inflector::camelize($this->context->id);?> <?= Html::aHelp(Yii::$app->params['helpUrl'][Yii::$app->language]['Recharges'] ?? null) ?></h2>
                <div class="card-tools">
                    <?= Html::create(['edit'], Yii::t('app', 'Recharge')) ?>
                    <?= Html::export() ?>
                    <!--<?= Html::import() ?>-->
                </div>
            </div>
            <div class="card-body">
                <?= GridView::widget([
                    ...
                    'columns' => [
                        ...
                        'remark',
                        // 'payment_method',
                        ['attribute' => 'payment_status', 'format' => 'raw', 'value' => function ($model) { return ActiveModel::getPaymentStatusLabels($model->payment_status); }, 'filter' => Html::activeDropDownList($searchModel, 'payment_status', ActiveModel::getPaymentStatusLabels(), ['class' => 'form-control', 'prompt' => Yii::t('app', 'Please Filter')]),],
                        'paid_at:datetime',
                        'amount',
                        // 'tax',
                        // 'invoice',
